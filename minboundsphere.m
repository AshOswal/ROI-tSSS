...
aset = 1:4; % arbitrary, but quite adequate
iset = 5:n;
...
while flag
  ...
  [center,radius] = enc4(xyz(aset,:));
  
  % are all the inactive set points inside the circle?
  ri = sqrt(sum((xyz(iset,:) - repmat(center,n-4,1)).^2,2));
  ...
  if (rmax - radius) <= tol
    ...
    % it seems best (a greedy algorithm) to swap it in.
