...
M     = spm_mesh_split(M,C==2);

%Fgraph  = spm_figure('GetWin', 'Comparison'); figure(Fgraph); clf
a =figure;
%layout = reshape(1:6,3,2);
ii=0;
for i1 = 1:2%size(layout,1)
    for i2 = 1:2%size(layout,2)
        v = spm_vol(ov{i2});
        P = spm_mesh_project(M, v);
        ii = ii+1;

        a = subplot(2,2,ii);
        H = spm_mesh_render('Disp', M, 'Parent', a);
        H = spm_mesh_render('Overlay', H, P);
        H = spm_mesh_render('ColourMap',H,jet);
        H = spm_mesh_render('clim',H,[0 600]);
        if i1==1
            view(90,0);
        elseif i1==2
            view(90,90);
        ...
        if ii==4
            ...
            H.colourbar.FontSize = 12;
            H.colourbar.Position = [0.02 0.02 0.02 0.3];
        end
