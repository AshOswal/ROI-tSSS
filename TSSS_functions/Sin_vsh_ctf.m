...
for j = 1:length(weights) % facilitates numerical integration
   ...
   cost = cos(theta);
   cosp = cos(phi);
   vs = vsh_modified_in(theta,phi,l,m)'/rn^(l+2);           % as per eq(6) in Taulu
   V(1,j) = vs(1)*sint*cosp + vs(2)*cost*cosp - vs(3)*sinp; % all x components conversion from sph to cartesian
   V(2,j) = vs(1)*sint*sinp + vs(2)*cost*sinp + vs(3)*cosp; % all y components
   V(3,j) = vs(1)*cost - vs(2)*sint;                        % all z components
end
