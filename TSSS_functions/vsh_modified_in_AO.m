...
% given in radians. 
%
function Vm = vsh_modified_in_AO(theta,phi,l,m)
...
Y = spharm_vec(theta,phi,l,m);
if m > -l
   Yminus = spharm_vec(theta,phi,l,m-1);
else
   Yminus = 0;
end
if m < l
   Yplus = spharm_vec(theta,phi,l,m+1);
else
   Yplus = 0;
end
...
Vm    = cat(3, scale*(-(l+1))*Y,  scale*dY,   scale*(i*m./sin(theta)).*Y);
Vm    = Vm.*(1/sqrt((l+1)*(2*l+1)));
%Vm    = sum(Vm.*Vm,3).*sin(theta);
